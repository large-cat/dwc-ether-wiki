...
> **Version**: 3.0
> **Architecture**: llm-wiki three-layer (raw/wiki/site) + tools/
> **Core Principle**: `raw/` is immutable. Only read from it on cache miss. Everything learned goes to `wiki/`.
...
│
├── wiki/                             ← LLM-managed knowledge (read/write)
...
│   ├── knowledge_data.json           ← Static QA pairs
│   └── leaves/                       ← Markdown exports of leaves (optional)
│
├── site/                             ← Built frontend (read-only)
│   └── (Vite builds to dist/, copy here for deployment)
│
├── tools/                            ← Knowledge growth engine
│   ├── knowledge_growth.py           ← ★ Main engine (search/read/cache/leaves)
...
| Layer | Contents | Mutability | Accessed By |
|-------|----------|------------|-------------|
| `raw/` | Source PDFs | **Immutable** | tools/knowledge_growth.py only |
| `wiki/` | Knowledge tree, cache, leaves, logs | **LLM-managed** | Claude Code, frontend, Python |
| `site/` | Built React SPA | **Build output** | Browser |
...
## Core Workflow (Lazy-Loading)

For each user question about DWC_ether_qos:
...
### answer_question(question, read_depth=5) → Dict
...
# result contains:
#   context         → Raw text from PDF/cache/leaves for you to synthesize
#   sources         → List of source references
#   leaves_found    → Knowledge leaves that matched
#   cache_hits      → Number of cache hits
#   cache_misses    → Number of times PDF was read
#   pdf_was_read    → True if PDF was accessed this query
#   new_docs        → Any new documents detected in raw/
```
...
5. **中文回答，英文术语.** Keep technical terms in English (RGMII, TSO, Descriptor), answer body in Chinese.
...
# → Returns context for synthesis

# 4. Claude synthesizes answer from result["context"]
# "RGMII使用4位数据通道，在时钟上升沿和下降沿传输数据..."

# 5. Save insight as leaf
...
# 6. Second question about same topic
result = answer_question("RGMII用什么时钟频率？")
# → [CACHE HIT] ch5_p167-171 → no PDF read!
# → find_leaves("RGMII时钟") → returns leaf_ch5_1
